#!/usr/bin/env python3
"""
Folder Organizer Pro v2.0
A powerful file organization tool with CustomTkinter GUI
Created by Oleksiy Gorbenko © 2025 profitsplusx.com
"""
...
class FolderRule:
    ...
    time_pattern: str = "YYYY-MM"  # YYYY-MM, YYYY-MMM, YYYY-MM-DD, etc
...
class FilterAction:
    ...
    action_type: str = "move_folder"  # move_folder, move_external, delete, exclude, rename
    destination: str = ""  # For move actions
    folder_name: str = ""  # For move to folder in structure
    rename_type: str = "prefix"  # prefix, suffix, custom
    rename_text: str = ""
    rename_pattern: str = ""  # For custom pattern
    priority: int = 0
...
class AppSettings:
    ...
    exclude_all_subfolders: bool = True
    exclude_specific_folders: List[str] = field(default_factory=lambda: ["node_modules", ".git", ".vscode", "__pycache__"])
    exclude_files: List[str] = field(default_factory=lambda: ["desktop.ini", "Thumbs.db", ".DS_Store"])
    exclude_file_types: List[str] = field(default_factory=lambda: [".exe", ".zip", ".rar"])
...
class FolderOrganizerApp(ctk.CTk):
    def __init__(self):
        ...
        self.title("Folder Organizer Pro v2.0")
    ...
    def create_settings_tab(self):
        ...
        # Create two columns
        left_frame = ctk.CTkFrame(scroll)
        left_frame.pack(side="left", fill="both", expand=True, padx=5)
        
        right_frame = ctk.CTkFrame(scroll)
        right_frame.pack(side="left", fill="both", expand=True, padx=5)
        
        # Left column - General Settings
        general_frame = ctk.CTkFrame(left_frame)
        ...
        ctk.CTkCheckBox(general_frame, text="Create backup log file",
                       ...
                       command=self.update_settings).pack(anchor="w", padx=20, pady=2)
        ...
        additional_frame = ctk.CTkFrame(right_frame)
        additional_frame.pack(fill="x", padx=5, pady=5)
        
        ctk.CTkLabel(additional_frame, text="Additional Options",
                    font=("Arial", 14, "bold")).pack(anchor="w", padx=5, pady=5)
        
        self.clean_empty_var = tk.BooleanVar(value=self.settings.clean_empty_folders)
        ctk.CTkCheckBox(additional_frame, text="Clean empty folders",
                       variable=self.clean_empty_var,
                       command=self.update_settings).pack(anchor="w", padx=20, pady=2)
        
        self.preserve_timestamps_var = tk.BooleanVar(value=self.settings.preserve_timestamps)
        ctk.CTkCheckBox(additional_frame, text="Preserve folder timestamps",
                       variable=self.preserve_timestamps_var,
                       command=self.update_settings).pack(anchor="w", padx=20, pady=2)
        
        # Analyze folder button
        analyze_btn = ctk.CTkButton(additional_frame, text="Analyze Folder",
                                    command=self.analyze_folder)
        analyze_btn.pack(anchor="w", padx=20, pady=10)
        
        # Exclusions section (spans both columns)
        ...
        self.exclude_subfolders_var = tk.BooleanVar(value=self.settings.exclude_all_subfolders)
        ctk.CTkCheckBox(exclude_frame, text="Exclude all subfolders",
                       variable=self.exclude_subfolders_var,
                       command=self.update_settings).pack(anchor="w", padx=20, pady=2)
        ...
        ctk.CTkLabel(exclude_frame, text="Exclude specific folders (comma-separated):").pack(anchor="w", padx=20, pady=(10, 2))
        self.exclude_folders_entry = ctk.CTkEntry(exclude_frame, width=400)
        self.exclude_folders_entry.pack(padx=20, pady=2)
        self.exclude_folders_entry.insert(0, ", ".join(self.settings.exclude_specific_folders))
        
        # Exclude files
        ctk.CTkLabel(exclude_frame, text="Exclude files (comma-separated):").pack(anchor="w", padx=20, pady=(10, 2))
        self.exclude_files_entry = ctk.CTkEntry(exclude_frame, width=400)
        self.exclude_files_entry.pack(padx=20, pady=2)
        self.exclude_files_entry.insert(0, ", ".join(self.settings.exclude_files))
        
        # Exclude file types
        ctk.CTkLabel(exclude_frame, text="Exclude file types (comma-separated):").pack(anchor="w", padx=20, pady=(10, 2))
        self.exclude_file_types_entry = ctk.CTkEntry(exclude_frame, width=400)
        self.exclude_file_types_entry.pack(padx=20, pady=2)
        self.exclude_file_types_entry.insert(0, ", ".join(self.settings.exclude_file_types))
    ...
    def create_status_bar(self):
        ...
        self.status_bar.pack(fill="x", padx=10, pady=(0, 2))
    
    def create_copyright_label(self):
        """Create copyright label at bottom"""
        copyright_label = ctk.CTkLabel(self, text="© 2025 profitsplusx.com", 
    ...
    def create_rule_widget(self, parent, rule: FolderRule, index: int):
        """Create an inline editable widget for a folder rule"""
        ...
        # Row 1: Enable checkbox and name
        row1 = ctk.CTkFrame(main_frame)
        row1.pack(fill="x", pady=2)
        
        enabled_var = tk.BooleanVar(value=rule.enabled)
        enabled_var.trace("w", lambda *args, idx=index: self.toggle_rule(idx, enabled_var.get()))
        ctk.CTkCheckBox(row1, text="", variable=enabled_var, width=30).pack(side="left", padx=5)
        
        ctk.CTkLabel(row1, text="Name:", width=60).pack(side="left")
        name_entry = ctk.CTkEntry(row1, width=200)
        name_entry.insert(0, rule.name)
        name_entry.bind("<FocusOut>", lambda e, idx=index: self.update_rule_field(idx, "name", name_entry.get()))
        name_entry.pack(side="left", padx=5)
        
        ctk.CTkLabel(row1, text="Level:", width=50).pack(side="left", padx=(20, 5))
        level_entry = ctk.CTkEntry(row1, width=50)
        level_entry.insert(0, str(rule.hierarchy_level))
        level_entry.bind("<FocusOut>", lambda e, idx=index: self.update_rule_field(idx, "hierarchy_level", int(level_entry.get()) if level_entry.get().isdigit() else 1))
        level_entry.pack(side="left")
        
        delete_btn = ctk.CTkButton(row1, text="Delete", width=60,
                                   fg_color="red", hover_color="darkred",
                                   command=lambda idx=index: self.delete_rule(idx))
        delete_btn.pack(side="right", padx=5)
        
        # Row 2: Type-specific fields
        row2 = ctk.CTkFrame(main_frame)
        row2.pack(fill="x", pady=2)
        
        if rule.is_time_based:
            ctk.CTkLabel(row2, text="Time Pattern:", width=100).pack(side="left", padx=5)
            
            pattern_var = tk.StringVar(value=rule.time_pattern)
            pattern_menu = ctk.CTkOptionMenu(row2, variable=pattern_var,
                                            values=["YYYY", "YYYY-MM", "YYYY-MMM", "YYYY-MM-Week#", "YYYY-MM-DD"],
                                            command=lambda val, idx=index: self.update_rule_field(idx, "time_pattern", val))
            pattern_menu.pack(side="left", padx=5)
            
            ctk.CTkLabel(row2, text="Date Source:", width=90).pack(side="left", padx=5)
            source_var = tk.StringVar(value=rule.date_source)
            source_menu = ctk.CTkOptionMenu(row2, variable=source_var,
                                          values=["modified", "created", "accessed"],
                                          command=lambda val, idx=index: self.update_rule_field(idx, "date_source", val))
            source_menu.pack(side="left", padx=5)
            
            include_year_var = tk.BooleanVar(value=rule.include_year)
            include_year_var.trace("w", lambda *args, idx=index: self.update_rule_field(idx, "include_year", include_year_var.get()))
            ctk.CTkCheckBox(row2, text="Include Year", variable=include_year_var).pack(side="left", padx=10)
        else:
            ctk.CTkLabel(row2, text="Folder:", width=60).pack(side="left", padx=5)
            folder_entry = ctk.CTkEntry(row2, width=150)
            folder_entry.insert(0, rule.folder_name)
            folder_entry.bind("<FocusOut>", lambda e, idx=index: self.update_rule_field(idx, "folder_name", folder_entry.get()))
            folder_entry.pack(side="left", padx=5)
            
            ctk.CTkLabel(row2, text="Extensions:", width=80).pack(side="left", padx=5)
            ext_entry = ctk.CTkEntry(row2, width=350)
            ext_entry.insert(0, ", ".join(rule.file_extensions))
            ext_entry.bind("<FocusOut>", lambda e, idx=index: self.update_rule_extensions(idx, ext_entry.get()))
            ext_entry.pack(side="left", padx=5)
    
    def update_rule_field(self, index, field, value):
        """Update a specific field of a rule"""
        if index < len(self.folder_rules):
            setattr(self.folder_rules[index], field, value)
            self.update_preview()
    
    def update_rule_extensions(self, index, extensions_text):
        """Update rule extensions from text"""
        if index < len(self.folder_rules):
            extensions = [ext.strip() for ext in extensions_text.split(",") if ext.strip()]
            self.folder_rules[index].file_extensions = extensions
            self.update_preview()
    ...
    def create_filter_widget(self, parent, filter_action: FilterAction, index: int):
        ...
        action_text = filter_action.action_type.replace("_", " ").title()
        ctk.CTkLabel(action_frame, text=f"Action: {action_text}",
                    font=("Arial", 10, "bold")).pack(anchor="w")
        
        if filter_action.folder_name:
            ctk.CTkLabel(action_frame, text=f"  Folder: {filter_action.folder_name}").pack(anchor="w")
        if filter_action.destination:
            ctk.CTkLabel(action_frame, text=f"  Destination: {filter_action.destination}").pack(anchor="w")
        if filter_action.action_type == "rename":
            if filter_action.rename_type == "custom":
                ctk.CTkLabel(action_frame, text=f"  Pattern: {filter_action.rename_pattern}").pack(anchor="w")
            else:
                ctk.CTkLabel(action_frame, text=f"  {filter_action.rename_type}: {filter_action.rename_text}").pack(anchor="w")
    ...
    def generate_preview_tree(self):
        ...
        for i, l1_rule in enumerate(level1_rules):
            ...
            if l1_rule.is_time_based:
                ...
                samples = self.get_time_folder_samples(l1_rule)
                for j, sample in enumerate(samples):
                    is_last_month = (j == len(samples) - 1) and is_last_l1
                    month_prefix = "└── " if is_last_month else "├── "
                    tree += f"    {month_prefix}📅 {sample}/\n"
                    ...
                    for k, l2_rule in enumerate(level2_rules):
                        ...
                        tree += f"    {indent}    {l2_prefix}📁 {l2_rule.folder_name}/\n"
                ...
                # Regular folder
                tree += f"    {prefix}📁 {l1_rule.folder_name}/\n"
    ...
    def get_time_folder_samples(self, rule):
        """Get sample time folder names based on pattern"""
        now = datetime.now()
        samples = []
        
        if rule.time_pattern == "YYYY":
            samples = ["2024", "2023", "2022"]
        elif rule.time_pattern == "YYYY-MM":
            samples = ["2024-09", "2024-08", "2024-07"]
        elif rule.time_pattern == "YYYY-MMM":
            samples = ["2024-Sep", "2024-Aug", "2024-Jul"]
        elif rule.time_pattern == "YYYY-MM-Week#":
            samples = ["2024-09-Week1", "2024-08-Week4", "2024-08-Week3"]
        elif rule.time_pattern == "YYYY-MM-DD":
            samples = ["2024-09-15", "2024-09-14", "2024-09-13"]
        else:
            samples = ["2024-09", "2024-08", "2024-07"]
        
        if not rule.include_year:
            samples = [s.split("-", 1)[1] if "-" in s else s for s in samples]
    ...
    def estimate_folders(self):
        ...
        for rule in self.folder_rules:
            if rule.enabled:
                if rule.is_time_based:
                    count += 3  # Assume 3 time periods for now
    ...
    def update_settings(self):
        ...
        self.settings.create_backup_log = self.backup_var.get()
        self.settings.show_confirmation = self.confirm_var.get()
        self.settings.delete_duplicates = self.duplicates_var.get()
        self.settings.clean_empty_folders = self.clean_empty_var.get()
        self.settings.preserve_timestamps = self.preserve_timestamps_var.get()
        self.settings.exclude_all_subfolders = self.exclude_subfolders_var.get()
    ...
    def show_about(self):
        ...
        # Logo/Title
        ctk.CTkLabel(about_window, text="Folder Organizer Pro v2.0",
                    font=("Arial", 18, "bold")).pack(pady=20)
        
        # Creator info
        ctk.CTkLabel(about_window, text="Created by Oleksiy Gorbenko",
                    font=("Arial", 12)).pack(pady=5)
        
        ctk.CTkLabel(about_window, text="© 2025 profitsplusx.com",
...
Features:
• Time-based organization with customizable patterns
...
• Backup and restore capabilities"""
        ...
        # Help button
        help_btn = ctk.CTkButton(about_window, text="Help Documentation",
                                command=lambda: webbrowser.open("https://github.com/OlekWebDev/File-Organizer"))
        help_btn.pack(pady=10)
        
        # Close button
        ctk.CTkButton(about_window, text="Close", 
                     command=lambda: self.close_window("about", about_window)).pack(pady=10)
        
        # Clean up reference when window is closed
        about_window.protocol("WM_DELETE_WINDOW", lambda: self.close_window("about", about_window))
    
    def close_window(self, key, window):
        """Close a window and remove its reference"""
    ...
    def save_config_to_file(self, file_path):
        ...
        self.settings.exclude_before_date = self.exclude_date_entry.get() or None
        self.settings.exclude_specific_folders = [f.strip() for f in self.exclude_folders_entry.get().split(",") if f.strip()]
        self.settings.exclude_files = [f.strip() for f in self.exclude_files_entry.get().split(",") if f.strip()]
        self.settings.exclude_file_types = [f.strip() for f in self.exclude_file_types_entry.get().split(",") if f.strip()]
    ...
    def load_config_from_file(self, file_path):
        ...
        try:
            ...
            if "settings" in config:
                ...
                # Update UI
                self.backup_var.set(self.settings.create_backup_log)
                self.confirm_var.set(self.settings.show_confirmation)
                self.duplicates_var.set(self.settings.delete_duplicates)
                self.clean_empty_var.set(self.settings.clean_empty_folders)
                self.preserve_timestamps_var.set(self.settings.preserve_timestamps)
                self.exclude_subfolders_var.set(self.settings.exclude_all_subfolders)
                self.exclude_date_entry.delete(0, "end")
                if self.settings.exclude_before_date:
                    self.exclude_date_entry.insert(0, self.settings.exclude_before_date)
                self.exclude_folders_entry.delete(0, "end")
                self.exclude_folders_entry.insert(0, ", ".join(self.settings.exclude_specific_folders))
                self.exclude_files_entry.delete(0, "end")
                self.exclude_files_entry.insert(0, ", ".join(self.settings.exclude_files))
                self.exclude_file_types_entry.delete(0, "end")
                self.exclude_file_types_entry.insert(0, ", ".join(self.settings.exclude_file_types))
    ...
    def analyze_folder(self):
        """Analyze target folder with proper window management"""
        ...
        # Check if window already exists
        if "analyze" in self.open_windows and self.open_windows["analyze"].winfo_exists():
            self.open_windows["analyze"].lift()
            self.open_windows["analyze"].focus()
            return
        ...
        # Store reference
        self.open_windows["analyze"] = result_window
        ...
        for ext, count in sorted_exts:
            if ext:
                ...
                info_text += f"  (no extension): {count} files\n"
        ...
        ctk.CTkButton(result_window, text="Close",
                     command=lambda: self.close_window("analyze", result_window)).pack(pady=10)
        
        # Clean up reference when window is closed
        result_window.protocol("WM_DELETE_WINDOW", lambda: self.close_window("analyze", result_window))
...
class FolderRuleDialog(ctk.CTkToplevel):
    """Dialog for adding new folder rules (only for new rules)"""
    
    def __init__(self, parent, rule: Optional[FolderRule] = None):
        super().__init__(parent)
        
        self.title("Add Folder Rule")
        ...
        self.is_time_based = tk.BooleanVar(value=False)
        ...
        self.level_entry = ctk.CTkEntry(form_frame, width=100)
        self.level_entry.insert(0, "1")
        self.level_entry.pack(anchor="w", pady=(0, 10))
        ...
        self.include_year_var = tk.BooleanVar(value=True)
        ctk.CTkCheckBox(self.time_frame, text="Include Year", variable=self.include_year_var).pack(anchor="w", pady=(0, 10))
    ...
    def save(self):
        ...
        if self.is_time_based.get():
            self.result = FolderRule(
                ...
                include_year=self.include_year_var.get()
            )
        else:
...
class FilterActionDialog(ctk.CTkToplevel):
    """Dialog for adding/editing filter actions with dynamic fields"""
    ...
    def __init__(self, parent, filter_action: Optional[FilterAction] = None):
        ...
        self.action_var = tk.StringVar(value="move_folder")
        actions = [
            ("Move to folder in structure", "move_folder"),
        ...
        self.action_fields_frame = ctk.CTkFrame(form_frame)
        self.action_fields_frame.pack(fill="x", pady=10)
        
        # Create all possible action fields (will be shown/hidden dynamically)
        
        # Folder name field (for move_folder)
        self.folder_frame = ctk.CTkFrame(self.action_fields_frame)
        ctk.CTkLabel(self.folder_frame, text="Folder Name:").pack(anchor="w", pady=(0, 2))
        self.folder_name_entry = ctk.CTkEntry(self.folder_frame, width=400,
                                              placeholder_text="Enter existing folder name or new folder to create")
        self.folder_name_entry.pack(fill="x")
        
        # Destination field (for move_external)
        self.dest_frame = ctk.CTkFrame(self.action_fields_frame)
        ...
        # Rename fields (for rename)
        self.rename_frame = ctk.CTkFrame(self.action_fields_frame)
        
        ctk.CTkLabel(self.rename_frame, text="Rename Type:").pack(anchor="w", pady=(10, 2))
        self.rename_type_var = tk.StringVar(value="prefix")
        
        rename_type_frame = ctk.CTkFrame(self.rename_frame)
        rename_type_frame.pack(fill="x", pady=(0, 10))
        
        ctk.CTkRadioButton(rename_type_frame, text="Prefix", 
                          variable=self.rename_type_var, value="prefix",
                          command=self.toggle_rename_type).pack(side="left", padx=10)
        ctk.CTkRadioButton(rename_type_frame, text="Suffix", 
                          variable=self.rename_type_var, value="suffix",
                          command=self.toggle_rename_type).pack(side="left", padx=10)
        ctk.CTkRadioButton(rename_type_frame, text="Custom", 
                          variable=self.rename_type_var, value="custom",
                          command=self.toggle_rename_type).pack(side="left", padx=10)
        
        # Prefix/Suffix text field
        self.rename_text_frame = ctk.CTkFrame(self.rename_frame)
        self.rename_text_label = ctk.CTkLabel(self.rename_text_frame, text="Prefix:")
        self.rename_text_label.pack(anchor="w", pady=(0, 2))
        self.rename_text_entry = ctk.CTkEntry(self.rename_text_frame, width=400)
        self.rename_text_entry.pack(fill="x")
        self.rename_text_frame.pack(fill="x", pady=(0, 10))
        
        # Custom pattern field
        self.rename_pattern_frame = ctk.CTkFrame(self.rename_frame)
        ctk.CTkLabel(self.rename_pattern_frame, text="Pattern (use {original} for filename):").pack(anchor="w", pady=(0, 2))
        self.rename_pattern_entry = ctk.CTkEntry(self.rename_pattern_frame, width=400,
                                                 placeholder_text="e.g., 2024_{original}_backup")
        self.rename_pattern_entry.pack(fill="x")
        ...
        if filter_action:
            ...
            if filter_action.folder_name:
                self.folder_name_entry.insert(0, filter_action.folder_name)
            if filter_action.destination:
                self.destination_entry.insert(0, filter_action.destination)
            if filter_action.rename_type:
                self.rename_type_var.set(filter_action.rename_type)
            if filter_action.rename_text:
                self.rename_text_entry.insert(0, filter_action.rename_text)
            if filter_action.rename_pattern:
                self.rename_pattern_entry.insert(0, filter_action.rename_pattern)
            self.priority_slider.set(filter_action.priority)
            self.update_priority_label(filter_action.priority)
        
        # Initialize action fields visibility
        self.toggle_action()
        if self.action_var.get() == "rename":
            self.toggle_rename_type()
    ...
    def toggle_action(self):
        ...
        if action == "move_folder":
            ...
            self.rename_frame.pack(fill="x", pady=10)
            self.toggle_rename_type()
    
    def toggle_rename_type(self):
        """Toggle rename type fields"""
        self.rename_text_frame.pack_forget()
        self.rename_pattern_frame.pack_forget()
        
        rename_type = self.rename_type_var.get()
        if rename_type == "prefix":
            self.rename_text_label.configure(text="Prefix:")
            self.rename_text_frame.pack(fill="x", pady=(0, 10))
        elif rename_type == "suffix":
            self.rename_text_label.configure(text="Suffix:")
            self.rename_text_frame.pack(fill="x", pady=(0, 10))
        elif rename_type == "custom":
            self.rename_pattern_frame.pack(fill="x", pady=(0, 10))
    ...
    def save(self):
        ...
        if action_type == "move_folder":
            ...
            if not folder_name:
                messagebox.showerror("Error", "Please enter a folder name")
                return
        elif action_type == "move_external":
            ...
            rename_type = self.rename_type_var.get()
            if rename_type in ["prefix", "suffix"]:
                rename_text = self.rename_text_entry.get().strip()
                if not rename_text:
                    messagebox.showerror("Error", f"Please enter a {rename_type}")
                    return
            else:  # custom
                rename_pattern = self.rename_pattern_entry.get().strip()
                if not rename_pattern or "{original}" not in rename_pattern:
                    messagebox.showerror("Error", "Please enter a valid pattern with {original}")
                    return
...
class FolderOrganizer:
    ...
    def generate_dry_run_report(self):
        ...
        for root, dirs, files in os.walk(self.target_folder):
            # Skip if excluding all subfolders and not in root
            if self.settings.exclude_all_subfolders and root != self.target_folder:
                continue
            
            # Skip specific excluded folders
            skip = False
            for excluded in self.settings.exclude_specific_folders:
                if excluded in root:
                    skip = True
                    break
            if skip:
            ...
            for file in files:
                ...
                # Check filters first (they have priority)
        ...
        report += f"Files to delete: {len(files_to_delete)}\n"
        report += f"Files excluded: {len(files_excluded)}\n"
                ...
                try:
                    ...
                    if filter_result:
                        if filter_result == "DELETE":
                            ...
                            # Create folder if it's a move_folder action
                            if not os.path.exists(filter_result):
                                os.makedirs(filter_result, exist_ok=True)
                                created_folders.add(filter_result)
                    ...
                    errors.append(f"Error with {file}: {str(e)}")
        
        # Clean empty folders if enabled
        if self.settings.clean_empty_folders:
            self.clean_empty_directories()
        
        report += f"Files moved: {moved_count}\n"
        report += f"Files deleted: {deleted_count}\n"
    ...
    def is_excluded(self, file_path):
        ...
        # Check exclude file types
        if file_ext in self.settings.exclude_file_types:
            return True
        ...
        for filter_action in sorted_filters:
            ...
            if matches:
                ...
                if filter_action.action_type == "delete":
                    ...
                    return "EXCLUDE"
                elif filter_action.action_type == "move_folder":
                    # Return path to folder in structure
                    return os.path.join(self.target_folder, filter_action.folder_name)
                elif filter_action.action_type == "move_external":
                    return filter_action.destination
                elif filter_action.action_type == "rename":
                    # Rename and continue processing
                    new_name = self.get_renamed_filename(filename, filter_action)
                    new_path = os.path.join(os.path.dirname(file_path), new_name)
                    if not self.dry_run:
                        os.rename(file_path, new_path)
                        self.log.append(f"Renamed: {filename} → {new_name}")
                    return None
        
        return None
    
    def get_renamed_filename(self, filename, filter_action):
        """Get renamed filename based on filter action"""
        name, ext = os.path.splitext(filename)
        
        if filter_action.rename_type == "prefix":
            return f"{filter_action.rename_text}{name}{ext}"
        elif filter_action.rename_type == "suffix":
            return f"{name}{filter_action.rename_text}{ext}"
        else:  # custom
            return filter_action.rename_pattern.replace("{original}", name) + ext
    
    def get_destination(self, file_path):
        """Get destination folder for file based on folder rules"""
        filename = os.path.basename(file_path)
        file_ext = os.path.splitext(filename)[1].lower()
        ...
        for level in range(1, 10):  # Support up to level 10
            ...
            for rule in level_rules:
                if rule.is_time_based:
                    ...
                    folder_name = self.format_time_folder(file_date, rule)
    ...
    def format_time_folder(self, date, rule):
        """Format time-based folder name"""
        if rule.time_pattern == "YYYY":
            result = date.strftime("%Y")
        elif rule.time_pattern == "YYYY-MM":
            result = date.strftime("%Y-%m")
        elif rule.time_pattern == "YYYY-MMM":
            result = date.strftime("%Y-%b")
        elif rule.time_pattern == "YYYY-MM-Week#":
            week_num = (date.day - 1) // 7 + 1
            result = date.strftime(f"%Y-%m-Week{week_num}")
        elif rule.time_pattern == "YYYY-MM-DD":
            result = date.strftime("%Y-%m-%d")
        else:
            result = date.strftime("%Y-%m")
        
        # Remove year if not included
        if not rule.include_year and "-" in result:
    ...
    def clean_empty_directories(self):
        ...
        for root, dirs, files in os.walk(self.target_folder, topdown=False):
            for dir_name in dirs:
                dir_path = os.path.join(root, dir_name)
                try:
                    if not os.listdir(dir_path):
                        os.rmdir(dir_path)
                        self.log.append(f"Removed empty folder: {dir_path}")
                except:
                    pass
